    ...
    <RoleGuard allowedRoles={["mother"]}>
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-4 md:space-y-8 pb-10 px-4 md:px-0">
        ...
        <div className="flex flex-wrap gap-2 md:gap-3 justify-start">
          ...
          <Link
            href="/dashboard/mother/me/analysis"
            className="group flex items-center gap-2 px-4 md:px-5 py-2 md:py-2.5 bg-white border border-slate-100 rounded-full shadow-sm hover:border-pink-300 hover:bg-pink-50 transition-all"
          >
            <div className="w-5 h-5 md:w-6 md:h-6 bg-pink-50 group-hover:bg-pink-500 rounded-full flex items-center justify-center transition-colors">
              <Heart className="w-2.5 h-2.5 md:w-3 md:h-3 text-pink-500 group-hover:text-white" />
            </div>
            <span className="text-[9px] md:text-[11px] font-bold text-slate-500 group-hover:text-pink-500 uppercase tracking-widest">
              Analisis AI Bunda
            </span>
            <ChevronRight className="w-3 h-3 md:w-3.5 md:h-3.5 text-slate-300 group-hover:text-pink-500 group-hover:translate-x-0.5 transition-all" />
          </Link>
        </div>
